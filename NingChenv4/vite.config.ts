...
export default defineConfig(({ mode }) => {
    ...
    const doubaoProxyApiKey = env.DOUBAO_API_KEY
    const doubaoProxyBaseUrl = env.VITE_DOUBAO_BASE_URL || 'https://ark.cn-beijing.volces.com/api/v3'
    ...
    server: {
        proxy: {
            ...
            // Local .env.local uses DOUBAO_API_KEY plus VITE_DOUBAO_API_KEY=local-dev-proxy.
            '/api/doubao': {
                target: doubaoProxyBaseUrl,
                changeOrigin: true,
                rewrite: path => path.replace(/^\/api\/doubao/, ''),
                secure: true,
                headers: doubaoProxyApiKey
                    ? { Authorization: `Bearer ${doubaoProxyApiKey}` }
                    : undefined,
            },
        },
    },
