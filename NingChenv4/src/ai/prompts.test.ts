...
import { buildDowagerReviewCommentPrompt, buildEmpressFeedbackPrompt, buildEmpressPrompt, buildFengDaozhiDraftPrompt, buildNorthSchemeParsePrompt, buildNpcFollowUpFinalPrompt, buildNpcPrompt, buildOmenEchoPrompt, buildSchemeFollowUpParsePrompt, buildSchemeNpcActionContext, buildSchemeNpcActionPrompt, sanitizeNpcReplyText } from './prompts'
...
describe('buildDowagerReviewCommentPrompt', () => {
    it('grounds the dowager comment in identity, score facts and plain text output', () => {
        const prompt = buildDowagerReviewCommentPrompt({
            poemTitle: '相见欢·林花谢了春红',
            poemLines: ['林花谢了春红，太匆匆。', '自是人生长恨水长东。'],
            mediumLabel: '作画',
            finalTierLabel: '合格',
            favorDelta: 20,
            evaluationSummary: '合格：林中花树、东流水、好景难留。',
            selectedEvidence: '定景：林中花树；景物：东流水；立意：好景难留',
        }).map(message => message.content).join('\n')

        expect(prompt).toContain('摄政太后')
        expect(prompt).toContain('萧宝颖从南朝前来投奔北周')
        expect(prompt).toContain('系统已先完成评分')
        expect(prompt).toContain('只输出太后一段纯文本短评')
        expect(prompt).toContain('不要写作者姓名')
        expect(prompt).toContain('验收档位：合格')
        expect(prompt).toContain('好感变化：+20')
...
describe('buildNorthSchemeParsePrompt', () => {
    ...
    it('includes related npc context so intrigue parsing can distinguish speaker and harmed target', () => {
        const npc = INITIAL_NPCS.find(item => item.name === '贺拔琪')!
        const relatedNpc = INITIAL_NPCS.find(item => item.id === 'yuwendi')!
        ...
        const prompt = buildNorthSchemeParsePrompt({
            ...
            relatedNpc,
            schemeType: 'slander',
            speech: '燕王借宗室名分催逼南征，实是要绕过帘前夺回诏令节制。',
            eventName: '南征廷议再起',
            eventBriefing: '帝党与后党围绕军令和中枢节制争执。',
        })[1].content

        expect(prompt).toContain('说话对象：贺拔琪')
        expect(prompt).toContain('牵连人物：宇文棣')
        expect(prompt).toContain('牵连人物类型：朝廷人物')
        expect(prompt).toContain('阵营偏向 偏帝党')
        expect(prompt).toContain('谗言/离间/嫁祸时，说话对象是入口，牵连人物才是被攻击或被牵连对象')
        expect(prompt).toContain('若说话对象与牵连人物派系不同')
    })
})
...
describe('buildFengDaozhiDraftPrompt', () => {
    ...
    it('keeps Feng Daozhi limited to player-visible information', () => {
        ...
        expect(prompt[1].content).toContain('萧宝颖当前暴露阶段：under_watch')
