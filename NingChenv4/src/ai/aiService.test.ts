...
describe('aiService', () => {
    ...
    it('uses the same-origin DeepSeek proxy when the production deployment is configured for server-side keys', async () => {
        delete (globalThis as any).window
        processEnv.VITE_DEEPSEEK_API_KEY = 'local-dev-proxy'
        processEnv.VITE_DEEPSEEK_BASE_URL = 'https://api.deepseek.com'
        processEnv.VITE_DEEPSEEK_MODEL = 'deepseek-v4-flash'
        delete processEnv.VITE_KIMI_API_KEY
        delete processEnv.VITE_KIMI_BASE_URL
        ...
        const fetchMock = vi.fn().mockResolvedValue({
            ok: true,
            json: async () => ({
                choices: [{ message: { content: 'production proxy ok' } }],
            }),
        })
        ;(globalThis as any).fetch = fetchMock
        ...
        const result = await chatCompletion(
            [{ role: 'user', content: 'use production proxy' }],
            { temperature: 0.2, maxTokens: 50, tag: 'test' },
        )
        ...
        expect(fetchMock).toHaveBeenCalledOnce()
        expect(fetchMock.mock.calls[0]?.[0]).toBe('/api/ai/chat/completions')
        expect(result).toBe('production proxy ok')
    })
