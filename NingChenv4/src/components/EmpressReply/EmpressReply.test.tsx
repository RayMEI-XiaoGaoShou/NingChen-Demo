...
describe('EmpressReply source contract', () => {
    it('routes the first-round empress reply guide through the Feng Daozhi dialogue overlay', () => {
        expect(empressReplySource).toContain("import { buildFirstRoundGuideSequence } from '../../game/fengDaozhiGuide'")
        expect(empressReplySource).toContain("import { FengDaozhiDialogueOverlay } from '../FengDaozhiDialogue/FengDaozhiDialogueOverlay'")
        expect(empressReplySource).toContain('<FengDaozhiDialogueOverlay')
        expect(empressReplySource).toContain("sequences={[buildFirstRoundGuideSequence('empress_reply')]}")
        expect(empressReplySource).toContain("onComplete={() => markFirstRoundGuideSeen('empress_reply')}")
    })
    ...
    it('generates and caches the empress reply before settlement', () => {
        ...
        expect(empressReplySource).toContain("import { useGameSfx } from '../../audio/gameSfx'")
        expect(empressReplySource).toContain('const { playSfx } = useGameSfx()')
        expect(empressReplySource).toContain("playSfx('empress-next-page')")
    ...
    it('keeps the title label, vertical seal, and art button from the letter system', () => {
        ...
        expect(empressReplySource).toContain('南陈 {formatReplySouthDimensionLabel(dimension)} {formatSignedDelta(rawValue)}')
