...
describe('GlobalAudio BGM scene routing', () => {
    ...
    it('turns off game BGM while the opening cinematic plays', () => {
        expect(resolveTrack({ prologueStep: 'OPENING_CINEMATIC', helpOverlayOpen: false, currentPhase: 'PROLOGUE', bgmSceneContext: null })).toBeNull()
        expect(globalAudioSource).toContain('setCurrentTrack(resolvedTrack)')
        expect(globalAudioSource).toContain('if (!currentTrack) {')
        expect(globalAudioSource).toContain('audio.pause()')
    })
