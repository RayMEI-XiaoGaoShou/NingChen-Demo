...
describe('SchemeFeedback orchestration', () => {
    ...
    it('routes the basic first-round guide through the Feng Daozhi dialogue overlay', () => {
        expect(schemeFeedbackSource).toContain("import { buildFirstRoundGuideSequence } from '../../game/fengDaozhiGuide'")
        expect(schemeFeedbackSource).toContain("import { FengDaozhiDialogueOverlay } from '../FengDaozhiDialogue/FengDaozhiDialogueOverlay'")
        expect(schemeFeedbackSource).toContain('<FengDaozhiDialogueOverlay')
        expect(schemeFeedbackSource).toContain("sequences={[buildFirstRoundGuideSequence('scheme_feedback')]}")
        expect(schemeFeedbackSource).toContain("onComplete={() => markFirstRoundGuideSeen('scheme_feedback')}")
        expect(schemeFeedbackSource).not.toContain('dialogue-skip')
        expect(schemeFeedbackSource).not.toContain("import { FirstRoundGuideModal } from '../FirstRoundGuide/FirstRoundGuideModal'")
        expect(schemeFeedbackSource).not.toContain('<FirstRoundGuideModal')
        expect(schemeFeedbackSource).not.toContain("import { SchemeOnboardingModal } from '../SchemePanel/SchemeOnboardingModal'")
        expect(schemeFeedbackSource).not.toContain('<SchemeOnboardingModal')
        expect(schemeFeedbackSource).not.toContain('first_follow_up_teaching')
    })
})
...
describe('SchemeFeedback', () => {
    it('shows the basic feedback guide before the follow-up onboarding on first entry', () => {
        ...
        expect(markup).not.toContain('first-round-guide-modal')
        expect(markup).not.toContain('feedback-follow-up')
    })

    it('does not show the old follow-up onboarding after the basic feedback guide has been read', () => {
        ...
        expect(markup).not.toContain('first-round-guide-modal')
        expect(markup).not.toContain('first-round-guide-actions')
    })
