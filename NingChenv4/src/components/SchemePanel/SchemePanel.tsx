...
import type { FengDaozhiDraftResult, FirstRoundGuideKey, GameDifficulty, NPC, OmenSpeechInput, SchemeType, SchemeAction } from '../../game/types'
...
export function getSchemeCardFirstRoundGuideKey(selectedScheme: SchemeType | null): FirstRoundGuideKey | null {
    switch (selectedScheme) {
        case 'advise':
            return 'scheme_card_advise'
        case 'slander':
            return 'scheme_card_slander'
        case 'alienate':
            return 'scheme_card_alienate'
        case 'frame':
            return 'scheme_card_frame'
        case 'proxy':
            return 'scheme_card_proxy'
        case 'secession':
        case 'rebellion':
            return 'scheme_card_secession'
        case 'omen':
            return 'scheme_card_omen'
        default:
            return null
    }
}
...
export function SchemeComposer(props: SchemeComposerProps = {}) {
    ...
    const schemePhaseFirstRoundGuide = currentRound === 1 && !firstRoundGuideSeen.scheme_phase
        ? buildFirstRoundGuideSequence('scheme_phase')
        : null
    const schemeCardFirstRoundGuideKey = getSchemeCardFirstRoundGuideKey(selectedScheme)
    const schemeCardFirstRoundGuide = currentRound === 1 &&
        schemeCardFirstRoundGuideKey &&
        firstRoundGuideSeen.scheme_phase &&
        !firstRoundGuideSeen[schemeCardFirstRoundGuideKey]
        ? buildFirstRoundGuideSequence(schemeCardFirstRoundGuideKey)
        : null
    const schemeFirstRoundGuideQueue = [
        schemePhaseFirstRoundGuide,
        schemeCardFirstRoundGuide,
    ].filter((sequence): sequence is NonNullable<typeof sequence> => Boolean(sequence))
    const schemeFirstRoundGuideKeys = [
        schemePhaseFirstRoundGuide ? 'scheme_phase' : null,
        schemeCardFirstRoundGuideKey && schemeCardFirstRoundGuide ? schemeCardFirstRoundGuideKey : null,
    ].filter((key): key is FirstRoundGuideKey => Boolean(key))
    ...
    return (
        <div className="scheme-composer scheme-composer-embedded">
            {schemeFirstRoundGuideQueue.length > 0 && (
                <FengDaozhiDialogueOverlay
                    sequences={schemeFirstRoundGuideQueue}
                    onSequenceComplete={() => undefined}
                    onComplete={() => {
                        schemeFirstRoundGuideKeys.forEach(key => markFirstRoundGuideSeen(key))
                    }}
                />
            )}
