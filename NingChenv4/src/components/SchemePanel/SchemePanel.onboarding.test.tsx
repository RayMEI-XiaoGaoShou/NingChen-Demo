...
describe('SchemePanel onboarding flows', () => {
    ...
    it('keeps first omen teaching available as Feng Daozhi dialogue data', () => {
        ...
        const joinedText = sequence.lines.map(line => line.text).join('\n')

        expect(sequence.lines.length).toBeGreaterThan(0)
        expect(sequence.lines.every(line => line.segmentLabel === '谶纬引导')).toBe(true)
        FIRST_OMEN_TEACHING_CONTENT.steps.forEach(step => {
            expect(joinedText).toContain(step)
        })
        FIRST_OMEN_TEACHING_CONTENT.impactNotes.forEach(note => {
            expect(joinedText).toContain(note)
        })
    })
})
