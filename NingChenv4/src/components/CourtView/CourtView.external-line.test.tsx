...
function renderCourtViewMarkupWithCurrentStore() {
    Object.assign(useGameStore.getInitialState(), useGameStore.getState())
    return renderCourtViewMarkup()
}

function buildFirstRoundGuideSeen(courtObserveSeen: boolean) {
    return {
        round_start: false,
        court_observe: courtObserveSeen,
        court_faction: false,
...
describe('CourtView layered game-screen flow', () => {
    ...
    it('queues the round one overview guide before the Feng Daozhi advisor kit', () => {
        useGameStore.setState({
            currentRound: 1,
            currentPhase: 'COURT_OBSERVE',
            firstRoundGuideSeen: buildFirstRoundGuideSeen(false),
            fengDaozhiGuideSeen: {},
        })

        const markup = renderCourtViewMarkupWithCurrentStore()
        const queueCallSource = courtViewSource.slice(
            courtViewSource.indexOf('buildCourtOverviewFengDaozhiQueue'),
            courtViewSource.indexOf('const handleCourtOverviewDialogueComplete'),
        )

        expect(markup).toContain('feng-daozhi-dialogue-backdrop')
        expect(courtViewSource).toContain('buildCourtOverviewFengDaozhiQueue')
        expect(courtViewSource).toContain('buildFirstRoundGuideSequence')
        expect(courtViewSource).toContain('buildFengDaozhiAdvisorKit')
        expect(courtViewSource).toContain('markFengDaozhiGuideSeenMany')
        expect(courtViewSource).toContain('FengDaozhiDialogueOverlay')
        expect(courtViewSource).not.toContain('dialogue-skip')
        expect(courtViewSource).not.toContain('handleSkip')
        expect(courtViewSource).not.toContain('FirstRoundGuideModal')
        expect(courtViewSource).not.toContain('FIRST_ROUND_GUIDE_CONTENT')
        expect(queueCallSource.indexOf('firstRoundGuide: courtOverviewFirstRoundGuide')).toBeGreaterThanOrEqual(0)
        expect(queueCallSource.indexOf('firstRoundGuide: courtOverviewFirstRoundGuide')).toBeLessThan(queueCallSource.indexOf('advisorKit,'))
    })

    it('wires the new first-round faction and NPC-detail guides through the Feng Daozhi overlay', () => {
        expect(courtViewSource).toContain("buildFirstRoundGuideSequence('court_faction')")
        expect(courtViewSource).toContain("buildFirstRoundGuideSequence('external_faction')")
        expect(courtViewSource).toContain("buildFirstRoundGuideSequence('npc_detail')")
        expect(courtViewSource).toContain("markFirstRoundGuideSeen('court_faction')")
        expect(courtViewSource).toContain("markFirstRoundGuideSeen('external_faction')")
        expect(courtViewSource).toContain("markFirstRoundGuideSeen('npc_detail')")
        expect(courtViewSource).toContain('courtScopedFirstRoundGuide')
        expect(courtViewSource).toContain('npcDetailFirstRoundGuide')
    })

    it('shows the Feng Daozhi advisor kit on later overview rounds when unseen', () => {
        useGameStore.setState({
            currentRound: 2,
            currentPhase: 'COURT_OBSERVE',
            firstRoundGuideSeen: buildFirstRoundGuideSeen(true),
            fengDaozhiGuideSeen: {},
        })
    ...
    it('does not auto-show a seen Feng Daozhi advisor kit again', () => {
        useGameStore.setState({
            currentRound: 2,
            currentPhase: 'COURT_OBSERVE',
            firstRoundGuideSeen: buildFirstRoundGuideSeen(true),
            fengDaozhiGuideSeen: { 'round:2:advisor_kit:v1': true },
        })
    ...
    it('keeps global risk state visible in the light HUD', () => {
        ...
        expect(markup).toContain('title="个人安危，取决于太后对萧宝颖的好感。')
    })
    ...
    it('ducks BGM while public statement and detail-entry voices are active', () => {
        ...
        expect(courtViewSource).toContain('detailVoicePlaying')
        expect(courtViewSource).toContain('setDetailVoicePlaying(true)')
        expect(courtViewSource).toContain('setDetailVoicePlaying(false)')
    ...
    it('waits for the NPC detail-entry voice before showing Feng Daozhi detail guidance', () => {
        const npcDetailOverlaySource = courtViewSource.slice(
            courtViewSource.indexOf('{npcDetailFirstRoundGuide && ('),
            courtViewSource.indexOf('{selectedNpc ? renderNpcDetail'),
        )

        expect(npcDetailOverlaySource).toContain('isPlaybackBlocked={detailVoicePlaying}')
        expect(npcDetailOverlaySource).toContain("onComplete={() => markFirstRoundGuideSeen('npc_detail')}")
    })
