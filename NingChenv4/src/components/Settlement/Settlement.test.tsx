...
describe('Settlement visual feedback contract', () => {
    ...
    it('centers the HUD status group against the full HUD and matches court HUD color rhythm', () => {
        ...
        expect(hudMarkup.indexOf('南征风向')).toBeLessThan(hudMarkup.indexOf('个人安危'))
    })
...
describe('Settlement source contract', () => {
    it('does not keep the old settlement first-round popup after the markdown guide moved it to empress reply', () => {
        expect(settlementSource).not.toContain("buildFirstRoundGuideSequence('settlement')")
        expect(settlementSource).not.toContain("markFirstRoundGuideSeen('settlement')")
        expect(settlementSource).not.toContain("import { FirstRoundGuideModal } from '../FirstRoundGuide/FirstRoundGuideModal'")
        expect(settlementSource).not.toContain('<FirstRoundGuideModal')
    })
