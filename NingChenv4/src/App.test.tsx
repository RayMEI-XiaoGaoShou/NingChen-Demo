...
describe('App phase loading contract', () => {
    it('lazy-loads the narrative entry and main round pages', () => {
        ...
        expect(appSource).toContain("lazy(() => import('./components/OpeningCinematic/OpeningCinematic')")
        expect(appSource).not.toContain("lazy(() => import('./components/Prologue/Prologue')")
        expect(appSource).not.toContain("lazy(() => import('./components/CharacterBios/CharacterBios')")
        ...
        expect(appSource).not.toContain('<GameplayGuide mode="entry" />')
        expect(appSource).not.toContain('<CharacterBios />')
        expect(appSource).not.toContain('<Prologue />')
    ...
    it('wraps page rendering in Suspense with a dedicated loading shell', () => {
        ...
        expect(appSource).toContain('信息摘要')
        expect(appSource).not.toContain('玩法说明')
    })
    ...
    it('keeps the local Doubao image key server-side in the Vite dev proxy', () => {
        expect(viteConfigSource).toContain('const doubaoProxyApiKey = env.DOUBAO_API_KEY')
        expect(viteConfigSource).toContain("const doubaoProxyBaseUrl = env.VITE_DOUBAO_BASE_URL || 'https://ark.cn-beijing.volces.com/api/v3'")
        expect(viteConfigSource).toContain("'/api/doubao'")
        expect(viteConfigSource).toContain("path.replace(/^\\/api\\/doubao/, '')")
        expect(viteConfigSource).toContain("Authorization: `Bearer ${doubaoProxyApiKey}`")
        expect(viteConfigSource).toContain("VITE_DOUBAO_API_KEY=local-dev-proxy")
    })
})
...
describe('App shell helpers', () => {
    ...
    it('hides the global header for in-game and narrative entry pages', () => {
        ...
        expect(shouldHideGlobalHeader('OPENING_CINEMATIC', 'PROLOGUE')).toBe(true)
