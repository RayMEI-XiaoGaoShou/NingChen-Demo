...
import { getCourtExposurePresentation, getInvasionPressurePresentation } from './pressureEngine'
import type {
    CampaignState,
    DelayedBacklash,
    Faction,
...
export interface FengDaozhiDraftContext {
    ...
    playerExposurePressureHint?: string
...
export function buildFengDaozhiDraftContext(params: {
    ...
    const dispositionHint = shouldUseCourtDispositionHint(request.schemeType)
        ...
        : null
    const playerExposure = getCourtExposurePresentation(request.playerSuspicionHeat ?? 0, request.playerDangerStage)
    ...
    return {
        ...
        playerDangerStage: request.playerDangerStage,
        playerExposurePressureHint: `${playerExposure.label}：${playerExposure.summary}`,
