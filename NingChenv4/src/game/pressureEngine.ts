...
export function getCourtExposurePresentation(
    heat: number,
    stage: PlayerDangerStage = 'safe',
): PressurePresentation {
    const normalized = clampPressure(heat)
    if (stage === 'under_review' || normalized >= 74) {
        return {
            label: normalized >= 88 ? '疑惧成势' : '案牍暗积',
            className: 'risk-critical',
            summary: '已有权臣把你的行止视作疑点。此项不再直接裁定生死，但会让朝堂计谋更难回旋。',
        }
    }
    ...
    if (stage === 'under_watch' || normalized >= 36) {
        return {
            label: '风声转紧',
            className: 'risk-warning',
            summary: '朝中已有眼线开始留意你。低风险试探、借他人之口行事，能缓和暴露压力。',
        }
    }
    ...
    return {
        label: '伪装尚稳',
        className: 'risk-safe',
        summary: '你的行止尚未引起成体系的追查，但高暴露说辞仍会让风声转紧。',
    }
}
