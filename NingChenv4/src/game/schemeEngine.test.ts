...
describe('schemeEngine contextual scheme rules', () => {
    ...
    it('routes cross-faction slander damage to the related court target instead of destabilizing the speaker faction', () => {
        const hebaqi = { ...INITIAL_NPCS.find(npc => npc.name === '贺拔琪')!, trust: 90 }
        const yuwendi = { ...INITIAL_NPCS.find(npc => npc.id === 'yuwendi')!, trust: 55 }

        const result = settleScheme(
            {
                id: 'empress-speaker-slanders-emperor-related',
                targetNpcId: hebaqi.id,
                schemeType: 'slander',
                relatedNpcId: yuwendi.id,
                playerSpeech: '燕王借宗室名分催逼南征，实是要绕过帘前夺回诏令节制；此事只可压他恩宠，不宜牵动后党自乱。',
                resolutionRoll: 0.02,
                northParse: makeNorthParse({
                    dominantIntent: 'divide',
        ...
        expect(result.factionEffects.emperor?.internalStability ?? 0).toBeLessThan(0)
        expect(result.factionEffects.empress?.internalStability ?? 0).toBe(0)
        expect(result.factionEffects.empress?.courtInfluence ?? 0).toBe(0)
        expect(result.relatedImpactSummary).toContain('宇文棣')
        expect(result.impactTrace?.factionEffectSummary.join('；')).toContain('帝党')
    })

    it('routes cross-faction alienation damage to the related court target instead of destabilizing the speaker faction', () => {
        const hebaqi = { ...INITIAL_NPCS.find(npc => npc.name === '贺拔琪')!, trust: 90 }
        const yuwendi = { ...INITIAL_NPCS.find(npc => npc.id === 'yuwendi')!, trust: 55 }

        const result = settleScheme(
            {
                id: 'empress-speaker-alienates-emperor-related',
                targetNpcId: hebaqi.id,
                schemeType: 'alienate',
                relatedNpcId: yuwendi.id,
                playerSpeech: '燕王与帝党士人各借皇帝名义争功，军令若由他二人分持，前线迟早彼此掣肘。',
                resolutionRoll: 0.02,
                northParse: makeNorthParse({
                    dominantIntent: 'divide',
                    militaryRelevance: 0.72,
                    governanceRelevance: 0.82,
                    socialOrderRelevance: 0.44,
                    fractureTransmission: 0.9,
                }),
            },
            hebaqi,
            yuwendi,
