...
function getIntro(npc: NPC): string {
    if (
        matchesNpc(npc, {
            ids: ['yuwendi', 'hebaqi', 'hebaqí', 'zuting', 'linghuelvguang', 'weichimu', 'weichimù'],
            names: ['宇文棣', '贺拔琪', '祖廷', '令狐律光', '尉迟暮'],
        })
    ) {
        return '以为，'
    }
    if (matchesNpc(npc, { ids: ['zongai'], names: ['宗艾'] })) return '私心以为，'
    if (
        matchesNpc(npc, {
            ids: ['hebabogui', 'hebaboguì', 'duguwenyue', 'erzhulie', 'erzhulié', 'ansiming'],
            names: ['贺拔伯圭', '独孤文约', '尔朱烈', '安思明'],
        })
    ) {
        return '看来，'
    }
...
function southDebate(npc: NPC): string {
    if (isNpc(npc, '宇文棣')) return '新朝方定，更该先声夺人，对陈不可再示弱。'
    if (isNpc(npc, '贺拔琪')) return '国政未稳，凡大战略都须握在朝廷节制之中。'
    if (isNpc(npc, '祖廷')) return '仓储、诏令、中枢未理顺，空谈南征，只会先乱自家。'
    if (isNpc(npc, '尉迟暮')) return '若朝廷真要打，兵、粮、令三样，缺一都不成。'
    if (isNpc(npc, '令狐律光')) return '国家根本未固，先安内而后图远，方是正道。'
    if (isNpc(npc, '宗艾')) return '陛下也在听众臣所言，谁忠谁佞，宫中自会记着。'
...
function huainanFriction(npc: NPC): string {
    if (isNpc(npc, '宇文棣')) return '淮南边衅既起，正说明对陈不可再以姑息了之。'
    if (isNpc(npc, '尉迟暮')) return '边线既动，就该先补兵补粮，别让前线空吃亏。'
    if (isNpc(npc, '祖廷')) return '流民、边报、转运都要控在中枢，方不致有人借题生事。'
    if (isNpc(npc, '宗艾')) return '边事轻重，总归要让陛下先听明白，再论谁是谁非。'
    if (isNpc(npc, '贺拔琪')) return '一场小摩擦而已，朝廷若自乱阵脚，才是真中敌计。'
    return npc.powerBase === 'external'
...
function springDrought(npc: NPC): string {
    if (isNpc(npc, '祖廷')) return '灾年先问仓储与赋役，不先稳百姓，朝廷拿什么谈远图。'
    if (isNpc(npc, '贺拔琪')) return '眼下最要紧的是定人心、稳朝纲，不可使灾情再生政变。'
    if (isNpc(npc, '令狐律光')) return '河北、河南既旱，妄动大战，只会先把国家拖垮。'
    if (isNpc(npc, '宇文棣')) return '赈灾自要做，但也不能因此把国家锐气一并做没了。'
    return npc.powerBase === 'external'
...
function northernBorder(npc: NPC): string {
    if (isNpc(npc, '令狐律光')) return '北边未稳，谁还鼓噪别线大战，便是不识轻重。'
    if (isNpc(npc, '尔朱烈') || isNpc(npc, '安思明')) return '边地既危，朝廷若要人出力，总该先让边军心里有数。'
    if (isNpc(npc, '宗艾')) return '北边军情，陛下不会不问，谁敢瞒上欺下，宫中自会记账。'
    if (isNpc(npc, '宇文棣')) return '北顾固要紧，可国家大势也不能被边风牵着走。'
    return npc.powerBase === 'external'
...
function westernFront(npc: NPC): string {
    if (isNpc(npc, '贺拔伯圭')) return '河西与蜀地这盘棋，终究还是得让懂西线的人来下。'
    if (isNpc(npc, '独孤文约')) return '平西线要的是章法，不是谁嗓门大，谁就配独揽大局。'
    if (isNpc(npc, '祖廷')) return '帅权可议，粮权与接管权却不可轻离中枢。'
    if (isNpc(npc, '贺拔琪')) return '谁能平事，朝廷自会用谁，但谁也别想借乱自成气候。'
    if (isNpc(npc, '宇文棣')) return '西线可救，却不能让旁人借此另立一座权势高台。'
    return npc.powerBase === 'external'
...
function internalCrisis(npc: NPC): string {
    if (isNpc(npc, '祖廷')) return '乱象既起，便该整仓、整簿、整人，先把国家机器重新拧紧。'
    if (isNpc(npc, '贺拔琪')) return '越是人心浮动，越不能任由流言与私议坏了朝廷分寸。'
    if (isNpc(npc, '宇文棣')) return '地方既坏成这样，更证明积弊已深，岂能再粉饰太平。'
    if (isNpc(npc, '令狐律光')) return '地方未宁、民心未定，谁若仍只顾争功，便是误国。'
    if (isNpc(npc, '宗艾')) return '宫中听得见外头风声，天命与人心，从来都不是小事。'
    return npc.powerBase === 'external'
...
function southernWar(npc: NPC): string {
    if (isNpc(npc, '尉迟暮')) return '前线既开，朝廷就该把兵粮与号令一并补足，别让将士空耗。'
    if (isNpc(npc, '宇文棣')) return '战既已开，朝廷更当同心，不可再让掣肘之论坏了军心。'
    if (isNpc(npc, '令狐律光')) return '救淮南可以，可都城与北边也不能因此全成空架子。'
    if (isNpc(npc, '祖廷')) return '久战之时，更见谁在耗国、谁在稳国，账都要一笔一笔算清。'
    if (isNpc(npc, '贺拔琪')) return '大战之际，更需有人压住全局，不可让人人只顾自家战功。'
    return npc.powerBase === 'external'
...
function regencyStruggle(npc: NPC): string {
    if (isNpc(npc, '宗艾')) return '归政之议，终究也要让陛下先明白谁真忠、谁借忠邀名。'
    if (isNpc(npc, '宇文棣')) return '归政本是正理，宗室若连这一步都不敢说，还谈什么社稷。'
    if (isNpc(npc, '贺拔琪')) return '国事未定，归政岂可只凭一时之气便妄动根本。'
    if (isNpc(npc, '祖廷')) return '归政须有次第，不可一纸空话便把政务系统全数搅乱。'
    return npc.powerBase === 'external'
        ? '朝中谁掌权，边地未必真在乎，只怕中枢借换局来掣地方。'
        : '名分之争最伤根本，谁若只图一时之胜，迟早要反噬。'
}

function isNpc(npc: NPC, name: string): boolean {
