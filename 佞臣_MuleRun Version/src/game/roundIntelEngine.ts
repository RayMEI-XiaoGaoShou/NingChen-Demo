...
export function buildAdvisorHint(
    ...
    if (aliveCore.length === 0) {
        return '冯道之密语：今朝明线已乱，先看谁还能站着说话，再择其裂隙下手。'
    }

    const leads = aliveCore.slice(0, 3).map(npc => {
        const reaction = (input.reactions[npc.id] ?? '各怀盘算，未必与表面言辞相同。').replace(/[。；]+$/g, '')
...
export function getRoundAdvisorHint(round: number, npcs: HintNpc[]): string {
    const intel = getRoundIntel(round)
    if (!intel) {
        return '冯道之密语：局势未明，先看谁最急、谁最稳，再顺着人心裂处落子。'
    }

    return buildAdvisorHint(
        {
...
export function getNpcRoundReaction(round: number, npc: NPC, _intelDepth = 0): string {
    const intel = getRoundIntel(round)
    const publicStatement = buildPublicStatement(intel?.theme, npc)
    const intelDepth = Math.max(0, _intelDepth)
...
function buildHintStatement(base: string, reaction: string | undefined, npc: NPC): string {
    ...
    return `${base} 至于真正会让${selfRef}${selfRef === npc.name ? '在意' : '上心'}的，多半还是${clue}。`
}

function buildDeepStatement(
    base: string,
    ...
    return `${base} ${selfRef}${selfRef === npc.name ? '心里真正盘算的' : '真正盘算的'}，往往还在${reactionClue}；至于${secretClue}这层旧账，${selfRef}${selfRef === npc.name ? '不会轻易忘' : '从不会轻易忘'}。`
}
...
function southDebate(npc: NPC): string {
    ...
    if (npc.id === 'hebaqí') return '国政未稳，凡大战略都须在朝廷掌握之中。'
    if (npc.id === 'zuting') return '仓廪、诏令、中枢未理顺，妄言南征，只会先乱自家。'
    if (npc.id === 'weichimù') return '若朝廷真要打，兵、粮、令三样，缺一样都不成。'
    if (npc.id === 'linghuelvguang') return '国家根本未固，先安内而后图远，方是正道。'
    if (npc.id === 'zongai') return '陛下也在听众臣所议，谁忠谁躁，宫中自会记着。'
...
function huainanFriction(npc: NPC): string {
    ...
    if (npc.id === 'weichimù') return '边线既动，就该先补兵补粮，别让前线空吃亏。'
    if (npc.id === 'zuting') return '流民、边报、转运皆要控在中枢，方不致有人借题生事。'
...
function springDrought(npc: NPC): string {
    if (npc.id === 'zuting') return '灾年先问仓廪与赈务，不先稳百姓，朝廷拿什么谈远图。'
    if (npc.id === 'hebaqí') return '眼下最要紧的是定人心、稳朝纲，不可使灾情再生政变。'
...
function northernBorder(npc: NPC): string {
    ...
    if (npc.id === 'erzhulié' || npc.id === 'ansiming') return '边地既危，朝廷若要人出力，总该先让边军心里有数。'
...
function westernFront(npc: NPC): string {
    if (npc.id === 'hebaboguì') return '河西与蜀地这盘棋，终究还是得让懂西线的人来下。'
    if (npc.id === 'duguwenyue') return '平西线要的是章法，不是谁嗓门大，谁就配独揽大局。'
    if (npc.id === 'zuting') return '帅权可议，饷权与接管权却不可轻离中枢。'
    if (npc.id === 'hebaqí') return '谁能平事，朝廷自会用谁，但谁也别想借乱自成气候。'
...
function internalCrisis(npc: NPC): string {
    if (npc.id === 'zuting') return '乱象既起，便该整仓、整籍、整人，先把国家机器重新拧紧。'
    if (npc.id === 'hebaqí') return '越是人心浮动，越不能任由流言与私议坏了朝廷分寸。'
    if (npc.id === 'yuwendi') return '地方既坏成这样，更证明积弊已深，岂能再粉饰太平。'
    if (npc.id === 'linghuelvguang') return '地方未宁、民心未定，谁若仍只顾争势，便是误国。'
    if (npc.id === 'zongai') return '宫中听得见外头风声，天命与人心，从来都不是小事。'
    return npc.powerBase === 'external'
        ? '朝中若借整顿之名把手伸过了界，边地也自有边地的难处。'
        : '乱局当前，最怕的不是病根重，而是人人都想借病收权。'
}

function southernWar(npc: NPC): string {
    if (npc.id === 'weichimù') return '前线既开，朝廷就该把兵粮与号令一并补足，别让将士空耗。'
    ...
    if (npc.id === 'hebaqí') return '大战之际，更需有人压住全局，不可让人人只顾自家战功。'
    return npc.powerBase === 'external'
        ? '主力既南顾，边地与地方自然更该先把自家根基守牢。'
        : '战事越大，越见朝堂谁能担事、谁只会争嘴。'
}
...
function regencyStruggle(npc: NPC): string {
    ...
    if (npc.id === 'hebaqí') return '国事未定，归政岂可只凭一时之气便妄动根本。'
    if (npc.id === 'zuting') return '归政须有次第，不可一纸空话便把政务系统全数搅乱。'
    return npc.powerBase === 'external'
        ? '朝中谁掌权，边地未必真在乎，只怕中枢借换局来碰地方。'
